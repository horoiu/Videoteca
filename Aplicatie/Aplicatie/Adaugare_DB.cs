...
namespace Aplicatie
{
    class Adaugare_DB
    {
        ...
        public static void inregistrez_domeniu_in_BD(string denumireDomeniu)
        {
            ...
            cmdAdaugFilm.CommandText = "INSERT INTO domenii(denumire) VALUES(@denumire_domeniu);";
            try
            {
        ...
        public static void inregistrez_film_in_BD(string denumireFilm, string imagineFilm, int idDomeniu, int anFilm, int totalFilme)
        {
            MySqlCommand cmdAdaugFilm = new MySqlCommand();
            cmdAdaugFilm.Connection = conn;
            cmdAdaugFilm.CommandText = "INSERT INTO filme(denumire, anul, imagine, nrtotal, nrdisponibile, iddomeniu) VALUES(@denumire_film, @anul, @imagine, @nrtotal, @nrtotal, @iddomeniu); ";
            try
            {
                conn.Open(); // Deschidem conexiunea cu Baza de Date
                ...
                cmdAdaugFilm.Parameters.AddWithValue("@iddomeniu", idDomeniu);
                cmdAdaugFilm.ExecuteNonQuery();
            }
            catch (Exception)
            {
